...
class _FakeExchange:
    ...
    async def load_markets(self):
        return self._mk
    async def fetch_ticker(self, gate_sym: str):
        return {"symbol": gate_sym, "last": 60000.0, "bid": 59990.0, "ask": 60010.0}
    async def fetch_balance(self):
        return {"BTC": {"free": 0.01}, "USDT": {"free": 1000}}
    async def create_order(self, gate_sym: str, typ: str, side: str, amount: float, price, params):
        self._last_id += 1
        oid = f"{self._last_id}"
        self._orders[oid] = {"id": oid, "symbol": gate_sym, "type": typ, "side": side, "amount": amount}
        return self._orders[oid]
    async def fetch_order(self, oid: str, gate_sym: str):
        return self._orders.get(oid, {})


@pytest.mark.asyncio
async def test_ccxt_adapter_min_flow(mock_settings):
