...
class OrchestratorFactory:
    ...
    def parse_symbols(settings: Any) -> list[str]:
        ...
        symbols_str = (
            getattr(settings, "SYMBOLS", "") or
            getattr(settings, "SYMBOL", "BTC/USDT")
        )
    ...
    @staticmethod
    async def auto_start_orchestrators(
        orchestrators: dict[str, Orchestrator],
        settings: Any
    ) -> None:
        ...
        for symbol, orchestrator in orchestrators.items():
            try:
                ...
                _log.info(
                    "orchestrator_auto_started",
                    extra={"symbol": symbol}
                )
            except Exception as e:
                _log.error(
                    "orchestrator_auto_start_failed",
                    extra={"symbol": symbol, "error": str(e)},
                    exc_info=True
                )
