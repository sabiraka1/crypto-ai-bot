...
async def handle_update(update: Dict[str, Any], cfg, bot, http) -> Dict[str, Any]:
    ...
    if text.startswith("/start"):
        reply = "Команды: /status, /why, /stats, /buy <s> (dev), /sell <s> (dev)"
    ...
    if text.startswith("/stats"):
        stats = uc_get_stats(cfg, bot, positions_repo=None, trades_repo=None)
        exp = stats.get("exposure_value", "0")
        pnl = stats.get("pnl_unrealized", "0")
        count = stats.get("positions_open", 0)
        reply = f"stats: positions={count}, exposure={exp}, pnl_unrealized={pnl}"
        return {"chat_id": chat_id, "text": reply}
    ...
    return {"chat_id": chat_id, "text": "Неизвестная команда. Есть: /status, /why, /stats, /buy <s> (dev), /sell <s> (dev)"} 
