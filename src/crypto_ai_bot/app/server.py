...
def _health_matrix(components: Dict[str, Dict[str, Any]]) -> Dict[str, Any]:
    """
    Word-матрица: healthy / degraded / unhealthy
...
def health() -> JSONResponse:
    ...
    drift_ms: Optional[int] = measure_time_drift(HTTP, urls=CFG.TIME_DRIFT_URLS or None, timeout=1.5)
...
def metrics_export() -> PlainTextResponse:
    """
    Дополняем экспорт системных метрик состояниями circuit breaker
    в формате, описанном в карте (broker_circuit_state + счётчики).
    """
    base = metrics.export()
...
def tick(body: Dict[str, Any] = Body(default=None)) -> JSONResponse:
    ...
    try:
        ...
        if tracker:
            try:
                # лёгкий трекер публикует гейджи; если есть — используем напрямую
...
def positions_open() -> JSONResponse:
    try:
        items = BOT.positions_repo.get_open() or []
    except Exception:
        items = []
...
def orders_recent(limit: int = Query(50, ge=1, le=500)) -> JSONResponse:
    try:
        items = BOT.trades_repo.list_recent(limit=limit) or []
    except Exception:
        items = []
    return JSONResponse({"status": "ok", "items": items})
