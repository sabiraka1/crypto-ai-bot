...
# фабрика брокера — по актуальному контракту
...
def health() -> JSONResponse:
    ...
    broker_detail = None
    try:
        # корректный вызов CB: заворачиваем аргументы в лямбду
        BREAKER.call(
    ...
    # drift: функция возвращает int|None
...
async def telegram_webhook(
    ...
    # адаптер ожидает broker и http-клиент
    resp = tg_adapter.handle_update(update, CFG, BROKER, HTTP)
