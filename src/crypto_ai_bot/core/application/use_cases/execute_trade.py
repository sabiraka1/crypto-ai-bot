...
async def execute_trade(
    *,
    ...
    """
    Execute trade with risk management and protective exits (side-aware).
    """
    # ── Market data
    ticker = await broker.fetch_ticker(symbol)
    mid_price = (
        (ticker.bid + ticker.ask) / Decimal("2")
        if ticker.bid and ticker.ask
        else (ticker.last or Decimal("0"))
    )
    spread_frac = (
        ((ticker.ask - ticker.bid) / mid_price)
        if ticker.bid and ticker.ask and mid_price > 0
        else Decimal("0")
    )
    ...
    recent_orders = (
        storage.trades.count_orders_last_minutes(symbol, 60)
        if getattr(storage, "trades", None) and hasattr(storage.trades, "count_orders_last_minutes")
        else 0
    )
    pnl_daily = (
        storage.trades.daily_pnl_quote(symbol)
        if getattr(storage, "trades", None) and hasattr(storage.trades, "daily_pnl_quote")
        else Decimal("0")
    )
    cooldown_sec = int(getattr(settings, "RISK_COOLDOWN_SEC", 0) or 0)
    cooldown_active = False
    try:
        cooldown_active = storage.positions.is_cooldown_active(symbol, cooldown_sec, now_ts_ms=now_ms())
    except Exception:
        cooldown_active = False

    # ── Выбор стороны до risk-check
    side = (side or "").lower()
    if side not in ("buy", "sell"):
        side = "buy" if position_base <= 0 else "sell"

    # ── Risk check (side-aware)
    if risk_manager and not force_action:
        fee_est = dec(str(getattr(settings, "FEE_PCT_ESTIMATE", "0")))
        slip_est = spread_frac
        r_inputs = RiskInputs(
        ...
        risk_result = risk_manager.check(r_inputs)  # sync
    ...
    if side == "buy":
        ...
        if getattr(storage, "idempotency", None) and hasattr(storage.idempotency, "check_and_store"):
            if not storage.idempotency.check_and_store(
                idem_key,
                ttl_sec=idempotency_ttl_sec,
                default_bucket_ms=idempotency_bucket_ms,
            ):
    ...
    try:
        if side == "buy":
            ...
            if order and order.filled:
                ...
                storage.positions.set_last_trade_ts(symbol, order.timestamp or now_ms())
