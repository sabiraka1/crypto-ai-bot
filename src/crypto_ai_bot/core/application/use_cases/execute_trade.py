...
async def execute_trade(
    *,
    ...
    settings: Optional[Any] = None,  # Добавляем settings для доступа к FEE_PCT_ESTIMATE
) -> Dict[str, Any]:
    """
    ...
    if risk_manager and not force_action:
        # Получаем оценку комиссии из настроек
        fee_estimate = getattr(storage.settings, 'FEE_PCT_ESTIMATE', dec("0.001")) if hasattr(storage, 'settings') else dec("0.001")
        ...
        risk_inputs = RiskInputs(
            ...
            est_fee_pct=fee_estimate,  # Добавлено
            est_slippage_pct=dec("0.001")  # Можно также брать из настроек
        )
