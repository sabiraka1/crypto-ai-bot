...
def inc_labels(name: str, labels: Dict[str, Any]) -> None:
    try:
        inc(name, **labels)  # type: ignore[misc]
    except Exception:
        inc(name)
    ...
    *,
    ...
    if idem_repo is not None:
        ...
        try:
            if not bool(idem_repo.check_and_store(key, ttl)):
                inc_labels("trade.blocked", {"reason": "idempotent_duplicate"})
        ...
        try:
            ...
            if bid > 0 and ask > 0:
                ...
                if spread_pct > max_slip_pct:
                    ...
                    inc_labels("trade.blocked", {"reason": "slippage"})
    ...
    try:
        if side == "buy":
            ...
            inc_labels("broker.order.create", {"side": "buy"})
            ...
            inc_labels("broker.order.create", {"side": "sell"})
        ...
        inc_labels("trade.failed", {"where": "broker"})
