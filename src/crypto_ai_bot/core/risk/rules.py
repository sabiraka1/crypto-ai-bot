...
def _parse_hours_spec(spec: str) -> List[Tuple[int, int]]:
    ...
    for p in parts:
        if "-" in p:
            a, b = p.split("-", 1)
            try:
                A = max(0, min(24, int(a))); B = max(0, min(24, int(b)))
                if A <= B:
                    out.append((A, B))
            except Exception:
                continue
            ...
            try:
                h = max(0, min(24, int(p))); out.append((h, h))
            except Exception:
                continue
    return out or [(0, 24)]
...
def check_time_sync(cfg: Any, http: Any) -> CheckResult:
    ...
    if drift_ms is None:
        return _ok("time_sync_unknown", **details)
    if ok:
        return _ok("time_sync_ok", drift_ms=int(drift_ms), limit_ms=limit, used=details.get("used"))
    return _bad("time_sync_drift_exceeded", drift_ms=int(drift_ms), limit_ms=limit, used=details.get("used"))
...
def check_spread(broker: Any, symbol: str, *, max_spread_bps: Optional[float] = None) -> CheckResult:
    ...
    limit = float(max_spread_bps if max_spread_bps is not None else 25.0)
...
def check_drawdown(trades_repo: Any, *, lookback_days: Optional[int] = None, max_drawdown_pct: Optional[float] = None) -> CheckResult:
    ...
    limit = float(max_drawdown_pct if max_drawdown_pct is not None else 10.0)
    try:
        pnl_pct = float(trades_repo.get_realized_pnl(days))
    except Exception:
