...
# читаем конфиг ТОЛЬКО через Settings (никаких os.getenv здесь)
from crypto_ai_bot.core.settings import Settings

# --- connect (fallback без прямого ENV) ---
try:
    from .sqlite_adapter import connect  # type: ignore
except Exception:
    def connect(db_path: str | None = None, **kwargs) -> sqlite3.Connection:
        """
        Fallback: путь берём из Settings().DB_PATH (или ':memory:'), а не из os.getenv.
        """
        if db_path is None:
            try:
                cfg = Settings()
...
# --- SqliteUnitOfWork: подхватываем разные названия, либо минимальный fallback ---
_UOW: Any = None
try:
...
if _UOW is None:
    class SqliteUnitOfWork:
        """Минимальная UoW-обёртка для sqlite3.Connection (BEGIN/COMMIT/ROLLBACK)."""
...
__all__ = [name for name in (
    "connect", "get_db_stats", "perform_maintenance", "in_txn", "SqliteUnitOfWork"
) if name in globals()]
