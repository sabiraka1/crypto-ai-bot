...
def _macd(values: list[float], fast: int = 12, slow: int = 26, signal: int = 9) -> tuple[float | None, float | None]:
...
def _bollinger(values: list[float], period: int = 20, k: float = 2.0) -> tuple[float | None, float | None, float | None]:
    if len(values) < period:
