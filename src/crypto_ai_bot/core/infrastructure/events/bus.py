...
class Event:
    """
    Унифицированное событие: минимальный переносимый формат.
    topic   — строковый канал (например, 'orders.executed')
    payload — произвольный JSON-совместимый словарь
    key     — опциональный ключ партирования/идемпотентности
...
class AsyncEventBus:
    ...
    def subscribe_dlq(self, handler: Handler) -> None:
        """Подписка на DLQ — получает события, не доставленные обработчикам ретраев."""
    ...
    async def publish(self, topic: str, payload: dict[str, Any], *, key: str | None = None) -> dict[str, Any]:
        """
        Публикация события с асинхронной доставкой локальным подписчикам.
        ...
        for h in handlers:
            ...
            while True:
                try:
                    await h(evt)  # передаём Event, а не payload
                    delivered += 1
