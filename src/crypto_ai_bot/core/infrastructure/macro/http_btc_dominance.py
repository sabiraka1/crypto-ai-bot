...
_log = get_logger("macro.http_btcdom")


class HttpBtcDomSource(BtcDomPort):
    """
    HTTP-источник BTC Dominance.
    Ожидается JSON вида: {"change_pct": -0.25}
    """
    ...
    async def change_pct(self) -> Optional[float]:
        try:
            ...
            _log.warning("btcdom_http_failed", extra={"url": self._url}, exc_info=True)
            return None
