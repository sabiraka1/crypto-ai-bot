...
"""
🛡️ RiskManager — лёгкий унифицированный риск-менеджер
Совместим с текущей архитектурой (Settings, Bot, PositionManager).

Особенности:
- ATR через crypto_ai_bot.analysis.get_unified_atr (с Wilder-fallback)
...
from crypto_ai_bot.analysis.technical_indicators import get_unified_atr as _get_unified_atr
...
class RiskMetrics:
    ...
    trend_strength: float      # наклон EMA20 (нормированный)
...
class RiskManager:
    ...
    def get_unified_atr(self, df: pd.DataFrame, period: Optional[int] = None) -> float:
        """
        Унифицированный ATR из общего модуля; если не удаётся — Wilder fallback.
        """
    ...
    def calculate_dynamic_stop_loss(
        ...
        """
        Строит SL от ATR с учётом режима рынка и границ процентов.
        Возвращает (sl_price, details).
        """
        atr = self.get_unified_atr(df)
