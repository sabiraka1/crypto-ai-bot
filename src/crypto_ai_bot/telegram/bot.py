...
-----------------------------
Полноценный Telegram-бот (вебхук) с широким набором команд.
Обновлено: импорт /train теперь первично ищет модель в analysis.sinyal_skorlayici,
а затем делает fallback на старый путь crypto_ai_bot.sinyal_skorlayici.
"""
...
# ---------- Robust import for training ----------
_train_model = None
try:
    from crypto_ai_bot.analysis.sinyal_skorlayici import train_model as _train_model  # предпочтительный путь
except Exception:
    try:
        from crypto_ai_bot.sinyal_skorlayici import train_model as _train_model  # backward compat
    except Exception:
        _train_model = None
...
def cmd_train(chat_id: str):
    if _train_model is None:
        send_telegram_message("Модуль обучения не найден. Положи файл в analysis/sinyal_skorlayici.py", chat_id)
        return
    try:
        result = _train_model()
        send_telegram_message(str(result), chat_id)
    except Exception as e:
...
def cmd_signals(chat_id: str):
    price = _last_price(SYMBOL)
    send_telegram_message(f"<b>Signals</b>\nprice={price:.2f}\n(rule/ai публикуются ботом в статусах)", chat_id)
