...
class RateLimiter:
    """Простой токен-бакет per-key (key = 'place_order:BTC/USDT:buy')."""
    def __init__(self, default_rps: float = 2.0, default_burst: float = 3.0):
        self.default_rps = float(default_rps)
        self.default_burst = float(default_burst)
        self._buckets: Dict[str, Tuple[float, float, float]] = {}  # key -> (tokens, ts, burst)
    ...
    def allow(self, key: str, *, rps: float | None = None, burst: float | None = None) -> bool:
        ...
        now = time.time()
        tokens, ts, b = self._buckets.get(key, (cap, now, cap))
