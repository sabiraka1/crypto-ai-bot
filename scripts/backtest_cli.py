...
# -------------------- Candle & loaders (совмещает ваши csv_loader/dataloader) --------------------
...
# -------------------- Strategy: EMA cross long-only (как у вас) --------------------
...
# -------------------- Engine (spot, long-only): buy → sell --------------------
    ...
    *,
    ...
    for c in candles:
        ...
        if sig["side"] == "buy" and pos_qty == 0.0:
            # покупаем на фиксированную сумму trade_size
            px = c.close * slip_buy
        ...
        if sig["side"] == "sell" and pos_qty > 0.0:
            ...
            # выручка от продажи: qty * px, комиссия уменьшит выручку
            gross = pos_qty * px
    ...
    # если позиция осталась открытой — закрывать не будем (строго long-only «как есть»)
